...
namespace al
{
	export inline auto fixhypot(fixed_t x, fixed_t y) -> fixed_t
	{
		return al_fixhypot(x, y);
	}

	export inline auto fixatan(fixed_t x) -> fixed_t
	{
		return al_fixatan(x);
	}

	export inline auto fixatan2(fixed_t y, fixed_t x) -> fixed_t
	{
