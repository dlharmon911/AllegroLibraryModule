...
namespace al
{
	...
	{
		return ALLEGRO::BITMAP(al_get_backbuffer(display.get()), al::internal::deleter_empty<ALLEGRO::BITMAP_DATA>);
	}
	...
	{
		return al_acknowledge_resize(display.get());
	}

	export inline bool resize_display(ALLEGRO::DISPLAY& display, ALLEGRO::SIZE<size_t> size)
	{
		return al_resize_display(display.get(), (int32_t)size.width, (int32_t)size.height);
	}
	...
	{
		return al_is_compatible_bitmap(bitmap.get());
	}
	...
	{
		return ALLEGRO::EVENT_SOURCE(al_get_display_event_source(display.get()), al::internal::deleter_empty<ALLEGRO::EVENT_SOURCE_DATA>);
	}

	export inline void set_display_icon(ALLEGRO::DISPLAY& display, const ALLEGRO::BITMAP& icon)
	{
		al_set_display_icon(display.get(), icon.get());
	}
	...
	export inline void set_display_icons(ALLEGRO::DISPLAY& display, const std::vector<ALLEGRO::BITMAP>& icons)
	{
		...
		al_set_display_icons(display.get(), (int32_t)temp.size(), (ALLEGRO::BITMAP_DATA**)temp.data());
	}
	...
	{
		al_set_window_position(display.get(), point.x, point.y);
	}
	...
	export inline ALLEGRO::POINT<int32_t> get_window_position(ALLEGRO::DISPLAY& display)
	{
		...
		al_get_window_position(display.get(), &point.x, &point.y);
	...
	{
		return al_set_window_constraints(display.get(), min.width, min.height, max.width, max.height);
	}
	...
	{
		return al_get_window_constraints(display.get(), &min.width, &min.height, &max.width, &max.height);
	}

	export inline void apply_window_constraints(ALLEGRO::DISPLAY& display, bool onoff)
	{
		al_apply_window_constraints(display.get(), onoff);
	}
	...
	{
		al_set_display_option(display.get(), option, value);
	}

	export inline int32_t get_display_option(const ALLEGRO::DISPLAY& display, int32_t option)
	{
		return al_get_display_option(display.get(), option);
	}
	...
	{
		al_acknowledge_drawing_halt(display.get());
	}

	export inline void acknowledge_drawing_resume(ALLEGRO::DISPLAY& display)
	{
		al_acknowledge_drawing_resume(display.get());
	}
